...
class Arcade extends CI_Controller {
    ...
    function index() {
                ...
                $this->user_model->updateStatus($_SESSION['user']->id,User::AVAILABLE);
    ...
    function acceptInvitation() {
	    ...
	    $match->user2_id = $hostUser->id;
	   	...
	   	//create an empty board for start
	   	$board = array();
	    for ($i=0; $i<=5; $i++){
	    	array_push($board,array(0,0,0,0,0,0,0));
	    }
	    
	    $match->board_state = json_encode(array("board"=>$board, "turn_player"=>rand(1,2)));	    
	...
	function declineInvitation() {
		...
		$this->user_model->updateStatus($invite->id,User::AVAILABLE);

		//update invite status to NULL both		
		$this->user_model->updateInvitation($invite->id, NULL);
